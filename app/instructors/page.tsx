    ...
    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${finished?"bg-slate-50 border-slate-100 opacity-50":"bg-white border-slate-200"}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[10px] text-slate-400 font-bold">
          {session.fields["Time"] ?? "—"} · {session.fields["Duration"] ?? "—"}
          {finished && <span className="ml-1 text-slate-300">done</span>}
        </p>
      </div>
    </div>
  )
                ...
                {instructors.map(inst => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSelected && <ChevronRight className="h-3.5 w-3.5 text-red-400 shrink-0" />}
                    </button>
                  )
                })}
              </div>
